...
import { ctaSnippetSchema } from "@/lib/cta/schemas";
...
export async function GET(request: Request) {
  try {
    ...
    const snippets = await prisma.ctaSnippet.findMany({
      where: {
...
export async function POST(request: Request) {
  try {
    const current = await requireCurrentUser();
    const parsed = ctaSnippetSchema.safeParse(await request.json().catch(() => null));
    ...
    const snippet = await prisma.ctaSnippet.create({
      data: {
        workspaceId: current.workspace.id,
