...
function runCheck(definition) {
  if (definition.name === 'Alipay live env') {
    return getAlipayCheckResult(definition)
  }

  const missing = definition.name === 'Tencent Hunyuan live env'
    ? getAiImageMissingFields()
    : definition.required.filter((key) => !hasValue(process.env[key]))
...
function getAlipayCheckResult(definition) {
  const missing = definition.required.filter((key) => !hasValue(process.env[key]))
  const privateKeyConfigured = hasValue(process.env.ALIPAY_PRIVATE_KEY_PEM) || hasValue(process.env.ALIPAY_PRIVATE_KEY_PATH)
  const publicKeyConfigured = hasValue(process.env.ALIPAY_PUBLIC_KEY_PEM) || hasValue(process.env.ALIPAY_PUBLIC_KEY_PATH)
  const certModeConfigured = (
    (hasValue(process.env.ALIPAY_APP_CERT_PEM) || hasValue(process.env.ALIPAY_APP_CERT_PATH)) &&
    (hasValue(process.env.ALIPAY_PUBLIC_CERT_PEM) || hasValue(process.env.ALIPAY_PUBLIC_CERT_PATH)) &&
    (hasValue(process.env.ALIPAY_ROOT_CERT_PEM) || hasValue(process.env.ALIPAY_ROOT_CERT_PATH))
  )

  const oneOfMissing = []
  if (!privateKeyConfigured) {
    oneOfMissing.push(['ALIPAY_PRIVATE_KEY_PEM', 'ALIPAY_PRIVATE_KEY_PATH'])
  }
  if (!publicKeyConfigured && !certModeConfigured) {
    oneOfMissing.push([
      'ALIPAY_PUBLIC_KEY_PEM',
      'ALIPAY_PUBLIC_KEY_PATH',
      'ALIPAY_APP_CERT_PEM+ALIPAY_PUBLIC_CERT_PEM+ALIPAY_ROOT_CERT_PEM',
...
function getAiImageMissingFields() {
  const provider = normalizeAiImageProvider(process.env.AI_IMAGE_PROVIDER)
  if (provider === 'demo') {
    return []
  }

  if (provider === 'jimeng') {
    return ['AI_IMAGE_PROVIDER', 'JIMENG_API_KEY', 'JIMENG_API_URL'].filter((key) => !hasValue(process.env[key]))
  }
...
function normalizeAiImageProvider(value) {
  return String(value || '').trim().toLowerCase() || 'tencent-hunyuan'
}
