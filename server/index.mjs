...
import { generateTitleSuggestions } from './ai-titles.mjs'
...
app.post('/api/ai/titles', aiLimiter, async (request, response) => {
  const { title, content, platform } = request.body ?? {}
  ...
  try {
    ...
    response.status(502).json({ message: error instanceof Error ? error.message : 'AI title generation failed.' })
  }
})
...
app.post('/api/payments/create-order', authenticate, paymentLimiter, async (request, response) => {
  ...
  try {
    const checkout = await createProviderCheckout({
      ...
      returnBaseUrl: request.get('origin') || resolveBaseUrl(request),
      clientIp: request.ip,
...
function hasAlipayLiveConfig() {
  return Boolean(
    ...
    (process.env.ALIPAY_PRIVATE_KEY_PEM || process.env.ALIPAY_PRIVATE_KEY_PATH) &&
    (
      process.env.ALIPAY_PUBLIC_KEY_PEM ||
      process.env.ALIPAY_PUBLIC_KEY_PATH ||
      (
        (process.env.ALIPAY_APP_CERT_PEM || process.env.ALIPAY_APP_CERT_PATH) &&
        (process.env.ALIPAY_PUBLIC_CERT_PEM || process.env.ALIPAY_PUBLIC_CERT_PATH) &&
        (process.env.ALIPAY_ROOT_CERT_PEM || process.env.ALIPAY_ROOT_CERT_PATH)
      )
    ),
  )
}
...
function hasAiImageConfig() {
  ...
  if (provider === 'jimeng') {
    return hasJiMengConfig()
  }

  if (provider === 'tencent-hunyuan') {
    return hasTencentHunyuanConfig()
  }

  return hasTencentHunyuanConfig() || hasJiMengConfig()
}
